...
if settings.ENABLE_INFERENCE:
    ...
    try:
        ...
        print("[router] ✓ Week 4 multimodal AI registered")
    except Exception as e:
        print(f"[router] ✗ Week 4 multimodal AI failed: {e}")
    
    # Week 4 - Clinical NLP (Saugata)
    try:
        from backend.api.routers.clinical_nlp import router as clinical_nlp_router
        app.include_router(clinical_nlp_router)
        print("[router] ✓ Week 4 clinical NLP registered")
    except Exception as e:
        print(f"[router] ✗ Week 4 clinical NLP failed: {e}")
