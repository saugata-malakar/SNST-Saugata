...
from backend.database.models import Doctor, Consultation
...
def find_available_doctor(db: Session, condition_type: str, mode: str) -> Doctor | None:
    _ = mode
    q = (
        db.query(Doctor)
...
def calculate_queue_position(db: Session) -> int:
    pending = db.query(Consultation).filter(Consultation.status == "pending").count()
    return pending + 1
